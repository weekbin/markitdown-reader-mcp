...
def _clear_pdf_slices(doc_name: str):
    """Clear only pdf_slices from index, leaving docx_slices intact."""
    index = _load_index(doc_name)
    if "pdf_slices" in index or "slices" in index:
        _log.debug(f"_clear_pdf_slices: clearing pdf_slices from index for {doc_name}")
        index["pdf_slices"] = []
        _save_index(doc_name, index)


def _clear_docx_slices(doc_name: str):
    """Clear only docx_slices from index, leaving pdf_slices intact."""
    index = _load_index(doc_name)
    if "docx_slices" in index or "slices" in index:
        _log.debug(
            f"_clear_docx_slices: clearing docx_slices from index for {doc_name}"
        )
        index["docx_slices"] = []
        _save_index(doc_name, index)
...
def _slice_pdf(
    src: str,
    doc_name: str,
    pages_per_slice: int = SLICE_PAGES,
    ...
    if force_refresh:
        _log.debug(
            f"_slice_pdf: force_refresh=True, clearing pdf_slices for {doc_name}"
        )
        _clear_pdf_slices(doc_name)
    ...
    with fitz.open(src) as doc:
        ...
        for i in range(0, len(doc), pages_per_slice):
            ...
            if elapsed >= time_budget:
                _log.debug(
                    f"  _slice_pdf: time budget exceeded at slice {i // pages_per_slice}, saving partial progress"
                )
                break
    ...
    _save_index(doc_name, index)
    _log.debug(
        f"  _slice_pdf: done, {len(slices)} slices, index saved, elapsed={time.monotonic() - start_time:.1f}s"
    )
    return slices
...
def _slice_docx(
    src: str,
    doc_name: str,
    blocks_per_slice: int = SLICE_BLOCKS,
    time_budget: float = 300.0,
    force_refresh: bool = False,
) -> list[dict]:
    ...
    if force_refresh:
        _log.debug(
            f"_slice_docx: force_refresh=True, clearing docx_slices for {doc_name}"
        )
        _clear_docx_slices(doc_name)
    start_time = time.monotonic()
    ...
    for i in range(0, len(blocks), blocks_per_slice):
        ...
        if elapsed >= time_budget:
            _log.debug(
                f"  _slice_docx: time budget exceeded at slice {i // blocks_per_slice}, saving partial progress"
            )
            break
        slice_idx = i // blocks_per_slice
        if slice_idx % 10 == 0:
            _log.debug(
                f"  _slice_docx: slice {slice_idx}/{total_slices}, mem={mem()}MB"
            )
    ...
    _save_index(doc_name, index)
    _log.debug(
        f"  _slice_docx: done, {len(slices)} slices, index saved, elapsed={time.monotonic() - start_time:.1f}s, mem={mem()}MB"
    )
    return slices
...
def _read_docx_text(path: str) -> str:
    ...
    for idx, elem in enumerate(doc.element.body):
        ...
        if tag == "p":
            text = "".join(t.text or "" for t in elem.iter(f"{{{docx_ns}}}t"))
            if text.strip():
                parts.append(text)
        elif tag == "tbl":
...
def _resize_image_if_needed(img_bytes: bytes, max_dim: int = 1280) -> bytes:
    ...
    try:
        ...
        out = io.BytesIO()
        save_fmt = (
            img.format if img.format in ("PNG", "JPEG", "GIF", "BMP", "WEBP") else "PNG"
        )
...
def _extract_images_from_pdf(
    ...
    _log.debug(
        f"_extract_images_from_pdf: doc={doc_name} page_start={starting_page} force_refresh={force_refresh}"
    )
    ...
    with fitz.open(pdf_path) as doc:
        ...
        for page_num in range(len(doc)):
            # page_range is 1-based (e.g. (1, 5) means pages 1–5), page_num is 0-based
            if page_range and (
                page_num < page_range[0] - 1 or page_num > page_range[1] - 1
            ):
                continue
            if page_num % 20 == 0:
                _log.debug(
                    f"  _extract_images_from_pdf: page {page_num+1}/{total_pages}, mem={mem()}MB"
                )
            page = doc[page_num]
            img_list = page.get_images(full=True)
            if not img_list and page_range is None:
                _log.warning(
                    f"_extract_images_from_pdf: page {page_num+1} has no images (img_list empty)"
                )
            img_rects = {}
            ...
            for img in img_list:
                ...
                if rects:
                    img_rects[xref] = (
                        rects[0].rect if hasattr(rects[0], "rect") else rects[0]
                    )
            ...
            for img_index, img in enumerate(img_list):
                ...
                if not img_bytes:
                    _log.warning(
                        f"_extract_images_from_pdf: xref={xref} page={page_num+1} has empty img_bytes"
                    )
                ...
                if is_new:
                    ...
                    if len(img_bytes) == 0:
                        _log.warning(
                            f"Skipping 0-byte image xref={xref} in page {page}"
                        )
                        continue  # Don't write to disk, don't add to index
                    out_path.write_bytes(img_bytes)
                    from .image import _is_small_image
                    ...
                    index["images"].append(
                        {
                            ...
                            "page": original_page,
                            "size": len(img_bytes),
                            "is_small": _is_small_image(str(out_path)),
                        }
                    )
                ...
                results.append(
                    {
                        ...
                        "path": str(out_path),
                        "page": original_page,
                        # Use the on-disk size (post-resize) for is_new images, or read
                        # the actual file size for duplicates so OCR thresholds are correct.
                        "size": (
                            len(img_bytes)
                            if is_new
                            else (
            ...
            _log.debug(
                f"_extract_images_from_pdf: page {page_num+1} processed, {len(img_list)} images found, cumulative={len(results)}"
            )

    _log.debug(
        f"  _extract_images_from_pdf: done, {len(results)} images, mem={mem()}MB"
    )
    return results, index


def _extract_images_from_docx(
    docx_path: str, doc_name: str, force_refresh: bool = False
) -> tuple[list[dict], dict]:
    ...
    with zipfile.ZipFile(docx_path, "r") as z:
        ...
        for rels_file in rels_files:
            ...
            for match in re.finditer(
                r'Id="(rId\d+)"[^>]*Target="media/([^"]+)"', rels_content
            ):
                rid = match.group(1)
                image_path = "word/" + match.group(2)
                rid_to_image[image_path] = rid
        _log.debug(
            f"  _extract_images_from_docx: {len(media_files)} media files, mem={mem()}MB"
        )
    ...
    _log.debug(
        f"  _extract_images_from_docx: done, {len(results)} images, mem={mem()}MB"
    )
    return results, index
